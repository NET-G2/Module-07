...
namespace DiyorMarket.Services
{
    public class ProductService : IProductService
    {
        ...
        public IEnumerable<ProductDto> GetProducts(ProductResourceParameters productResourceParameters)
        {
            ...
            if (!string.IsNullOrWhiteSpace(productResourceParameters.OrderBy))
            {
                switch (productResourceParameters.OrderBy.ToLower())
                {
                    case "name":
                        query.OrderBy(p => p.Name); break;
                    case "namedesc":
                        query.OrderByDescending(p => p.Name); break;
                    case "description":
                        query.OrderBy(p => p.Description); break;
                    case "descriptiondesc":
                        query.OrderByDescending(p => p.Description); break;
                    case "price":
                        query.OrderBy(p => p.Price); break;
                    case "pricedesc":
                        query.OrderByDescending(p => p.Price); break;
                    case "expireDate":
                        query.OrderBy(p => p.ExpireDate); break;
                    case "expireDatedesc":
                        query.OrderByDescending(p => p.ExpireDate); break;
                    default: query.OrderBy(p => p.Name); break;
                }
            }
